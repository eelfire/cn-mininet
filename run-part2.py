    ...
    for congestion in ['vegas', 'reno', 'cubic', 'bbr']:
        if config == 'd':
            for link_loss in [1, 3]:
                print(
                    f"Running mininet-tcp.py with config {config} and congestion {congestion} and link_loss {link_loss}")
            ...
            print(
                f"Running mininet-tcp.py with config {config} and congestion {congestion}")
